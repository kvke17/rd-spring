  ...
  if (items.length === 0) {
    return (
      <div className="min-h-screen bg-[#0a0a0a] text-white flex flex-col items-center justify-center p-6">
        ...
        <Link href="/catalogo" className="bg-[#FF0000] text-black font-bold px-6 py-3 uppercase tracking-wide hover:bg-opacity-90">Explorar Catálogo</Link>
      </div>
    );
  }
  ...
  const handlePayment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch('/api/checkout', {
        ...
        body: JSON.stringify({
          // 🚨 SEGURIDAD: Eliminamos el 'amount' de aquí. 
          // Ahora enviamos el 'shippingId' para que el servidor calcule el total real.
          buyOrder,
        ...
        <form onSubmit={handlePayment} className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          ...
          <div className="lg:col-span-7 space-y-10">
            ...
            <div className="border border-white/10 bg-[#121212] p-6 sm:p-8">
              <h2 className="text-xs uppercase tracking-widest text-[#FF0000] mb-6 font-bold">01 · TIPO DE DOCUMENTO</h2>
              <div className="flex gap-4 mb-6">
                <button type="button" onClick={() => setDocType('BOLETA')} className={`flex-1 py-3 text-xs font-bold font-mono tracking-widest border transition-all ${docType === 'BOLETA' ? 'bg-[#FF0000] text-black border-[#FF0000]' : 'bg-transparent text-gray-400 border-white/20 hover:border-white/50'}`}>
                  BOLETA
                </button>
                <button type="button" onClick={() => setDocType('FACTURA')} className={`flex-1 py-3 text-xs font-bold font-mono tracking-widest border transition-all ${docType === 'FACTURA' ? 'bg-[#FF0000] text-black border-[#FF0000]' : 'bg-transparent text-gray-400 border-white/20 hover:border-white/50'}`}>
                  FACTURA
                </button>
              </div>
              ...
              {docType === 'FACTURA' && (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 pt-2">
                  <div>
                    <label className="block text-[11px] uppercase tracking-widest text-[#FF0000] mb-2 font-mono">Razón Social</label>
                    <input type="text" required={docType === 'FACTURA'} value={formData.razonSocial} onChange={(e) => setFormData({ ...formData, razonSocial: e.target.value })} className="w-full bg-[#0a0a0a] border border-white/10 p-3 text-sm focus:border-[#FF0000] focus:outline-none text-white font-mono" />
                  </div>
                  <div>
                    <label className="block text-[11px] uppercase tracking-widest text-[#FF0000] mb-2 font-mono">Giro Comercial</label>
                    <input type="text" required={docType === 'FACTURA'} value={formData.giro} onChange={(e) => setFormData({ ...formData, giro: e.target.value })} className="w-full bg-[#0a0a0a] border border-white/10 p-3 text-sm focus:border-[#FF0000] focus:outline-none text-white font-mono" />
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="border border-white/10 bg-[#121212] p-6 sm:p-8">
              <h2 className="text-xs uppercase tracking-widest text-[#FF0000] mb-6 font-bold">02 · DATOS DEL CLIENTE</h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                  <label className="block text-[11px] uppercase tracking-widest text-gray-400 mb-2 font-mono">NOMBRE COMPLETO</label>
                  <input type="text" required value={formData.fullName} onChange={(e) => setFormData({ ...formData, fullName: e.target.value })} className="w-full bg-[#0a0a0a] border border-white/10 p-3 text-sm focus:border-[#FF0000] focus:outline-none text-white font-mono" />
                </div>
                <div>
                  <label className="block text-[11px] uppercase tracking-widest text-gray-400 mb-2 font-mono">EMAIL</label>
                  <input type="email" required value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} className="w-full bg-[#0a0a0a] border border-white/10 p-3 text-sm focus:border-[#FF0000] focus:outline-none text-white font-mono" />
                </div>
                <div>
                  <label className="block text-[11px] uppercase tracking-widest text-gray-400 mb-2 font-mono">TELÉFONO</label>
                  <input type="tel" required value={formData.phone} onChange={(e) => setFormData({ ...formData, phone: e.target.value })} className="w-full bg-[#0a0a0a] border border-white/10 p-3 text-sm focus:border-[#FF0000] focus:outline-none text-white font-mono" />
                </div>
                <div>
                  ...
                  <input
                    ...
                    className={`w-full bg-[#0a0a0a] border p-3 text-sm focus:outline-none font-mono text-white ${rutError ? 'border-red-500' : 'border-white/10 focus:border-[#FF0000]'}`}
                  />
                ...
                <div className="sm:col-span-2">
                  ...
                  <input type="text" placeholder="Ej: Porsche 911 991.2 · 2018" value={formData.vehicle} onChange={(e) => setFormData({ ...formData, vehicle: e.target.value })} className="w-full bg-[#0a0a0a] border border-white/10 p-3 text-sm focus:border-[#FF0000] focus:outline-none text-white font-mono" />
                </div>
              </div>
            </div>

            <div className="border border-white/10 bg-[#121212] p-6 sm:p-8">
              <h2 className="text-xs uppercase tracking-widest text-[#FF0000] mb-6 font-bold">03 · ENTREGA</h2>
              <div className="space-y-3">
                {SHIPPING_OPTIONS.map((option) => (
                  <label key={option.id} onClick={() => setShippingOption(option.id)} className={`flex items-center justify-between p-4 cursor-pointer border transition-all ${selectedShippingId === option.id ? 'border-[#FF0000] bg-[#FF0000]/5 text-white' : 'border-white/10 bg-[#0a0a0a] text-gray-300 hover:border-white/20'}`}>
          ...
          <div className="lg:col-span-5">
            <div className="border border-white/10 bg-[#121212] p-6 sm:p-8 sticky top-28">
              <h2 className="text-xs uppercase tracking-widest text-[#FF0000] mb-6 font-bold">RESUMEN DEL PEDIDO</h2>
              ...
              <div className="space-y-3 pt-4 border-t border-white/10 font-mono text-xs">
                ...
                <div className="flex justify-between text-base font-bold text-white pt-4 border-t border-white/10"><span className="font-sans uppercase tracking-wider">Total</span><span className="text-[#FF0000]">{STORE_CONFIG.CURRENCY_FORMAT.format(getCartTotal())}</span></div>
              </div>
              <button type="submit" disabled={loading} className="w-full mt-8 bg-[#FF0000] text-black font-bold py-4 uppercase tracking-wider hover:bg-opacity-90 transition text-sm disabled:opacity-50">
